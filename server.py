...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_UPLOAD:
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is True:
            ...
            connection_socket.send(
                # bug-7: 408 ---> 402
                make_response_packet(OP_UPLOAD, 402, TYPE_FILE,
        ...
        if os.path.exists(join('tmp', username, json_data[FIELD_KEY])) is False:
            ...
            connection_socket.send(
                # bug-8: 408 ---> 404
                make_response_packet(OP_UPLOAD, 404, TYPE_FILE,
                                     f'The "key" {json_data[FIELD_KEY]} is not accepted for uploading.',
                                     {}))
            return
    ...
    if request_operation == OP_DOWNLOAD:
        ...
        if block_index < 0:
            ...
            connection_socket.send(
                # bug-12: 410 ---> 406
                make_response_packet(OP_GET, 406, TYPE_FILE, f'The "block_index" should >= 0.', {}))
            return
    ...
    while True:
        ...
        if request_operation == OP_LOGIN:
            if request_type != TYPE_AUTH:
                ...
                # bug-13: 'password' ---> FIELD_PASSWORD
                if hashlib.md5(json_data[FIELD_USERNAME].encode()).hexdigest().lower() != json_data[FIELD_PASSWORD].lower():
                    connection_socket.send(
        ...
        user_str = ".".join(token.split('.')[:3])  # 0-2
        md5_auth_str = token.split('.')[3]  # 3
        ...
        # bug-4: data_process ---> file_process
...
def tcp_listener(server_ip, server_port):
    ...
    while True:
        try:
            ...
            # bug-3: STEP_service ---> step_service
